    ...
    if (tmpEta == -1 ) { // need to interpolate    
      for (Int_t iEta = 0; iEta < nBinsEta-1; ++iEta){								       								         
        if (rightEta[iEta] <= TMath::Abs(eta) && TMath::Abs(eta) <leftEta[iEta+1] ){													         
 	        if (sigmaPhiSigmaEta >= 1)  tmpInter = ( fcorr_PHOTONS[iEta]  .Eval(sigmaPhiSigmaEta) + 															          
 		         	    		         fcorr_PHOTONS[iEta+1].Eval(sigmaPhiSigmaEta) ) / 2. ;														          
 	        else tmpInter = (xcorr_PHOTONS[iEta] + xcorr_PHOTONS[iEta+1])/2.; 
        }																						         
      }																						         
      return tmpInter;																					         
    }  																							         
    if (sigmaPhiSigmaEta >= 1) return fcorr_PHOTONS[tmpEta].Eval(sigmaPhiSigmaEta);						       						       
    else return xcorr_PHOTONS[tmpEta]; 
 } 
